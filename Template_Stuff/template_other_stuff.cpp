...
struct modnum
{
    ...
    v_t v;
    modnum() : v(0) {}
    modnum(vv_t _v) : v(v_t(_v % MOD))
    {
        if (v < 0)
            v += MOD;
    }
    explicit operator v_t() const { return v; }
    friend std::istream &operator>>(std::istream &i, modnum &n)
    {
        vv_t w;
        i >> w;
        n = modnum(w);
...
void solve()
{
    ...
    foaL(i, 2, k * 2 + 2)
    {
        mn temp = mn(3).pow(n + 1 - i);
        ll j = i / 2;
        ll j2 = (i + 1) / 2;
        temp *= mn::ncr(k - 1, j - 1);
        temp *= mn::ncr(n - k, j2 - 1);
        ans += temp;
    }
    ps(ans);
}
