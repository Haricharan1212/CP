...
void solve()
{
    ...
    rep(i, 0, n)
    {
        ...
        {
            int cnt = 0;
            dfs(adj, i, subtree_size, visited, cnt);

            int centroid = i;
            check(adj, centroid, subtree_size, visited2, cnt);
            centroids.push_back(centroid);
        }
    }

    sort(centroids.begin(), centroids.end(), greater<int>());

    vector<pi> ans;
    rep(i, 0, centroids.size() - 1)
    {
        ans.push_back({centroids[i + 1], centroids[0]});
        adj[centroids[i + 1]].push_back(centroids[0]);
        adj[centroids[0]].push_back(centroids[i + 1]);
    }

    cout << getdia(adj) << endl;
